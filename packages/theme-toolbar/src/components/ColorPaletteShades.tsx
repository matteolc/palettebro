              ...
              {[50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950].map(
                (variant) => {
                  const hint: string | undefined =
                    hints[`${color}-${variant}` as keyof typeof hints];
                  ...
                  return (
                    <div
                      ...
                      style={
                        {
                          '--tw-ring-color': `oklch(var(--${color}-200)/10)`,
                        } as React.CSSProperties
                      }
                    >
                      <div
                        className={clsx(
                          'h-20 rounded-[inherit] border-b border-[0.5px] p-2 leading-tight text-xs',
                        )}
                        style={{
                          backgroundColor: `oklch(var(--${color}-${variant}))`,
                          color: `oklch(var(--${color}-${textVariant}))`,
                        }}
                      >
                      ...
                      {hint && (
                        <div className="absolute bottom-0 left-1/2 flex w-full -translate-x-1/2 translate-y-full flex-col items-center justify-center p-2">
                          ...
                          <p className="w-full text-center text-xs text-neutral-800">
                            {hint}
                          </p>
                        </div>
                      )}
