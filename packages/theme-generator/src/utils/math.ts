export const round = (number: number, precision: number) =>
  number ? Number.parseFloat(number.toFixed(precision)) : number;
...
export const interval = (min: number, max: number, ratio: number) =>
  min + (max - min) * ratio;

export const circularInterval = (min: number, max: number, ratio: number) => {
  const circularClockwise = circular(max - min);
  const circularCounter = circular(min - max);
  const shortest =
    circularClockwise < circularCounter ? circularClockwise : -circularCounter;

  return circular(min + shortest * ratio);
};

export const closestAngle = (angle: number, target: number, step: number) => {
