      ...
      {showStatePalette ? (
        <div className="mx-4 flex flex-row gap-1">
          {STATUS_TOKENS.map((token) => (
            isLoading ? <Skeleton key={token} className="h-[52px] w-48 rounded-lg bg-zinc-200"/> : <ColorSwatch
              token={token}
              key={token}
        ...
        <div className="mx-4 flex flex-row gap-1">
          {BASE_TOKENS.map((token) => (
            isLoading ? <Skeleton key={token} className="h-[52px] w-48 rounded-lg bg-zinc-200"/> : <ColorSwatch token={token} key={token} />
          ))}
        </div>
      )}
